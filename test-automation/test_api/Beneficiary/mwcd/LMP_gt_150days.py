...
class test_Beneficiary_registration:
    ...
    def test_registration(self):
        ...
        payload['BeneficiaryRegistrationDate'] = "2019-06-16"
        payload['LMPDate'] = "2020-06-06"
        ...
        assert response.json()['Benresp']['Errors'][0][
                   'ErrMsg'] == 'Date of LMP must not exceed the Beneficiary Registration Date. Please enter the correct date.'
        assert response.json()['Benresp']['Errors'][1][
                   'ErrMsg'] == 'LMP Date must not exceed Date of Reg. of MCP card at AWC/Subcenter and Registration Date'
        assert response.json()['Benresp']['Errors'][2][
                   'ErrMsg'] == 'Date of Reg of MCP card must not exceed the Beneficiary Registration Date. Please enter the correct date.'
        assert response.json()['Benresp']['Errors'][3][
                   'ErrMsg'] == 'Date of Reg of MCP must not be earlier than LMP date.'

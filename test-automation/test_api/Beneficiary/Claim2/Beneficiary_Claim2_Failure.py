...
class test_Beneficiary_registration:
    ...
    def test_registration_claim2_failure(self):
        print("Generate claim2 payload")
        with open(str(Path(__file__).parents[1]) + "\\payloads\\claim2.json")as f1:
        ...
        payload['ClaimDate'] = "2019-10-10"
        endpoint = self.url
        print ("Accessing the endpoint: {}".format(self.url))
        response = requests.post(self.url, data=json.dumps(payload))
        print("Response: {}".format(response.json()))
        assert response.json()['status'] == 'failure'
        errors = response.json()['Benresp']['Errors']
        for errmsg in errors:
            assert 'ERR' in errmsg['ErrCode']
            print (errmsg['ErrMsg'])
...
if __name__ == '__main__':
    test_Beneficiary_registration().test_registration_claim2_failure()
